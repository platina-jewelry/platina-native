...
- **Axios:**  
  Библиотека для работы с HTTP-запросами в `React Native`. Axios позволяет отправлять запросы к API, обрабатывать ответы
...
- **Swift (iOS) и Kotlin (Android):** Несмотря на мощь и гибкость `React Native`, есть случаи, когда необходимо
  ...
  `React Native`. Для таких случаев мы используем _Swift_ для _iOS_ и _Kotlin_ для _Android_. Эти языки позволяют нам
